...
void NDMenuMovie::init()
{
	...
	//------------------------------------------
	/*
	      Texture[] movie_border_TEXTURE = ms<Texture>[13];
	      ...
	      sp<SDL_Surface> movie_YUV_SURFACE[4];
	      SMPEG_Info movieInfo[4];
	      SMPEG* movie[4];
	
	      sp<SDL_Surface> movie_RGBA_SURFACE = null;
	      ...
	      //for(m=0;m<4;m++)
	      {
	         sp<SDL_Surface> tempSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, movieInfo[m].width, movieInfo[m].height, 32, rmask, gmask, bmask, amask);
